...
| 1 | Game_State | Int | `1`: Tic-Tac-Toe (Stage 1)
`2`: Reaction Game (Stage 2)
`3`: Whac-A-Mole (Stage 3) |
| 2 | P1_Ready | Int | `0`: Waiting
`1`: Ready (Button Pressed) |
| 3 | P2_Ready | Int | `0`: Waiting
`1`: Ready (Button Pressed) |
...
| --- | --- | --- | --- |
| 0 | Header | String | `$TTT` |
| 1 - 9 | Board[0]..[8] | **0 / 1 / 2** | **0**: Empty
**1**: Player 1 (O)
**2**: Player 2 (X)
*(Note: Firmware handles internal -1 conversion)* |
| 10 | Current_Player | **1 / 2** | **1**: Player 1's turn
**2**: Player 2's turn |
| 11 | Winner | **0 / 1 / 2** | **0**: In Progress / Draw
**1**: Player 1 Wins
**2**: Player 2 Wins |
...
| 6 | Winner | Int | `-1`: In Progress
`1`: Player 1 (Closer to target)
`2`: Player 2 (Closer to target) |
| 7 | Time_Tick | Long | Game timer (Unit: 100us). |
| 8 | **P1_State** | **Int** | **0**: Waiting / Idle
**1**: **Rolling** (Number is changing, press to stop)
**2**: **Locked** (Turn finished, result fixed) |
| 9 | **P2_State** | **Int** | **0**: Waiting / Idle
**1**: **Rolling** (Number is changing, press to stop)
**2**: **Locked** (Turn finished, result fixed) |
